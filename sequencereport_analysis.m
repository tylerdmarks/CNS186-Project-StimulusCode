...
for ii = 1:num_subjects
    ...
    % exclude outliers
    ft_inf = ft_inf(ft_inf <= 10);
    ft_sup = ft_sup(ft_sup <= 10);
    tt_inf = tt_inf(tt_inf <= 15);
    tt_sup = tt_sup(tt_sup <= 15); 
...
difficulty_edges = [0 1.1 1.9 10];                % easy, medium, hard edges
...
for ii = 1:num_subjects
    ...
    performance = zeros(1, length(expt.sequence));
    seq_difficulty = zeros(1, length(expt.sequence));
    for ss = 1:length(expt.sequence)
        % find percent correct
        correct = expt.sequence{ss} == response.sequence{ss};
        performance(ss) = sum(correct)/length(correct);
        % calculate difficulty of the sequence
        u = length(unique(expt.sequence{ss}));      % number of unique elements in the sequence
        c = max(diff([0 find(diff(expt.sequence{ss})) numel(expt.sequence{ss})]));       % max number of consecutive elements
    ...
    for dd = 1:length(difficulty_edges)-1
        diffcurve_inf(ii, dd) = mean(performance(expt.field == 0 & binned_difficulty == dd));   % inf field, curr difficulty
        diffcurve_sup(ii, dd) = mean(performance(expt.field == 1 & binned_difficulty == dd));   % sup field, curr difficulty
    end
    
    performance_inf(ii) = mean(performance(expt.field == 0));
    performance_sup(ii) = mean(performance(expt.field == 1));
end
